...
Agent mode and MCP servers are Copilot's most powerful features — and the fastest way to burn through AI Credits. An unoptimized agent loop can consume **10x more tokens** than the same task done in Edit mode.
...
Understanding this loop explains why Agent mode costs **5–50K tokens** per task: it runs this entire cycle multiple times. Each iteration compounds the cost of every previous step.
...
Before mid-2025, all schemas loaded every step. With Tool Search (Module 9), only needed tools load. Pruning unused MCP servers still matters — it shrinks the registry and reduces the chance of the model picking expensive tools for simple tasks.

See [Module 9 — Tool Search](/token-economy-course/en/09-under-the-hood) for how it works internally.
:::
...
At Claude Sonnet input pricing ($3/M input), a single heavy 10-step agent task costs ~$0.30 — from tool registry overhead alone.
...
Not every task needs the same model. A systematic router saves **73% in daily API costs** — reducing average spend from $15/day to $4/day — by matching task complexity to model tier.
...
1. **Keep the model selector on Auto** — Copilot routes simple requests to cheaper models automatically, saving ~60-70% on eligible turns
...
About 80% of chat requests are simple enough for a lightweight model. If you leave the selector on a premium model "just in case," you're overpaying on 4 out of 5 requests.
:::
...
**Evaluator-Optimizer** saves the most tokens in practice. Catching an error in review costs 2,000 tokens. Fixing that same error after it reaches production costs 50,000+ tokens across debugging, refactoring, and re-review. The 2-3 cycle overhead pays for itself on the first caught bug.
:::
...
See [Module 2 — Cache-Busting Mistakes](/token-economy-course/en/02-context-model#common-cache-busting-mistakes) for more patterns that break the cache.
...
Agent hygiene builds on [Project Setup](/token-economy-course/en/06-project-setup). For a broader view of managing AI-generated code quality, see [Vibe Coding Without the Debt](/token-economy-course/en/08-vibe-coding-guardrails).
:::
