...
The VS Code team found that output compression alone reduces billable output tokens by **~5.5%** on average. Tool search (deferring tool schemas) saves an additional **~9–11%** by loading full tool definitions only when needed. Combined with code-only instructions (Module 4), output tokens can drop by 70%+.
...
In 2026, GitHub Copilot in VS Code moved to **persistent WebSocket connections** for agentic sessions. This doesn't directly reduce token count, but it reduces **time-to-first-token by ~19%** (median TTFT) and eliminates the TLS handshake overhead on every turn.
...
pie title Where Token Savings Come From<br/>(Remaining harness optimizations, per session)
    "Tool search + output compression" : 30
...
- **WebSocket connections** reduce time-to-first-token by ~19% (median TTFT), making sessions faster and less likely to be abandoned
