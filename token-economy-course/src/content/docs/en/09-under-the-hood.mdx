...
For common cache-busting mistakes and how to avoid them, see [Module 2 — Common Cache-Busting Mistakes](/vibe-on-budget/en/02-context-model#common-cache-busting-mistakes).
:::
...
Use the [Cache Explorer view](https://code.visualstudio.com/docs/agents/agent-troubleshooting/cache-explorer) in Agent Debug Logs to see your actual cache hit rates and how many input tokens were reused. This closes the optimization loop from Chapter 3: measure with Cache Explorer → adjust one variable → measure again.
...
:::note[Module 7 covers the practical cost model]
Module 7's MCP cost estimates represent the **pre-Tool Search baseline** — they're still useful as a worst-case scenario and for understanding *why* pruning matters. With Tool Search, actual costs are often 60–75% lower than those estimates.

See [Module 7 — Agents, Tools & MCP Hygiene](/vibe-on-budget/en/07-agents-mcp) for practical MCP management.
:::
...
Internal optimizations make the most sense after understanding the [context model](/vibe-on-budget/en/02-context-model) and [cost structure](/vibe-on-budget/en/01-hidden-tax). For practical ways to complement these savings, see [Agents, Tools & MCP Hygiene](/vibe-on-budget/en/07-agents-mcp).
:::
