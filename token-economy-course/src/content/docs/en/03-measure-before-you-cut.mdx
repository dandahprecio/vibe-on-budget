...
*(See [Glossary](/vibe-on-budget/en/glossary) for optimization loop and quality threshold definitions.)*

---

## Per-request credit hover 🟢

The fastest way to inspect a request is to hover over the Copilot response in Chat. VS Code shows the **credit consumption** for that turn.[^m3-2] This is the cost of one request, not the total cost of the conversation.
...
Per-request cost is useful, but it does not show the full bill for a long agent session. The **[context window control](https://code.visualstudio.com/docs/chat/copilot-chat)** in the Chat input field provides the cumulative view.[^m3-3] Hover over it or click it to inspect the total credits and token breakdown for the current session.
...
The [**Copilot status bar**](https://code.visualstudio.com/docs/agents/optimize-usage-guide#_copilot-status-dashboard) in VS Code gives you a monthly perspective. Select it to open the status dashboard, where you can see how much of your monthly allowance you have used.[^m3-4] The dashboard includes AI credits and, for Copilot Free, inline suggestions.
...
Read more about querying session history with Chronicle in the [VS Code documentation](https://code.visualstudio.com/docs/agents/run/sessions/session-history#_query-session-history-with-chronicle).[^m3-1]
...
For detailed evidence, enable file logging in VS Code settings:[^m3-5]
...
## [Cache Explorer view](https://code.visualstudio.com/docs/agents/agent-troubleshooting/cache-explorer) 🟢

Agent Debug Logs also include **Cache Explorer**. It shows prompt cache hit rates and how many input tokens were reused.[^m3-6] This matters because two sessions with similar token counts can have different costs when one reuses more cached input.
...
1. Pick a task you do regularly (e.g., "add a validation function"). Note the prompt you used, the model, credits consumed (hover), and whether the result needed a retry. This is your baseline.
2. Open Agent Debug Logs → Summary for your last session. Write down: total tokens, tool calls, errors, and duration. Which number surprised you most?
3. Run `/chronicle:cost-tips` in any chat. Pick ONE recommendation and test it on your next task.
...
Turn your baseline into action with [Quick Wins](/vibe-on-budget/en/04-quick-wins), then improve your prompts with [Writing Prompts That Cost Less](/vibe-on-budget/en/05-cheaper-prompts).
:::
...
[^m3-2]: [Optimize AI credit usage in VS Code](https://code.visualstudio.com/docs/agents/guides/optimize-usage) — VS Code Documentation. Describes the per-turn credit consumption shown for individual requests.
[^m3-3]: [Manage agent sessions in VS Code](https://code.visualstudio.com/docs/agents/run/sessions/manage-sessions) — VS Code Documentation. Describes the context window control and its session credit and token breakdown.
[^m3-4]: [Optimize AI credit usage in VS Code](https://code.visualstudio.com/docs/agents/guides/optimize-usage) — VS Code Documentation. Describes the Copilot status bar and monthly usage view.
[^m3-5]: [Debug chat interactions](https://code.visualstudio.com/docs/agents/agent-troubleshooting/chat-debug-view) — VS Code Documentation. Documents Agent Logs and the Chat Debug view for inspecting requests and tool invocations.
[^m3-6]: [Diagnose prompt caching with the Cache Explorer](https://code.visualstudio.com/docs/agents/agent-troubleshooting/cache-explorer) — VS Code Documentation. Documents cache hits, misses, and reused input tokens.
