...
Do not assume that HTML comments are stripped before injection. Daniel Vaughan reports comment stripping for [Codex CLI](https://github.com/openai/codex), while a [Claude Code](https://docs.anthropic.com/en/docs/claude-code) issue reports that `CLAUDE.md` comments are sent verbatim.[^m6-4][^m6-6] Chakrabarti's study reports that prompt comments reduced excess instruction growth by **99.3%** in its evaluated setting.[^m6-3]
