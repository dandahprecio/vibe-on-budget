...
Cached tokens are dramatically cheaper than fresh tokens. A useful mental model is **~10% of fresh cost on Anthropic** (about a 90% discount). OpenAI also offers automatic prefix caching but does not publish a specific discount percentage — savings vary by workload and prefix stability. That means a **10K-token session with 60% cache hits can be cheaper than a 5K-token session with zero hits**.
:::
...
|----------|-----------|-------------------|------------|----------|-------------|
| OpenAI (GPT-5.x) | Auto + Manual | Yes — `prompt_cache_breakpoint` | 1.25x | Not publicly specified | Configurable |
...
OpenAI GPT-5.x supports configurable cache retention (24h default for non-ZDR orgs on GPT-5 series models). Cache is best-effort, not guaranteed — a call can occasionally land on a server without your prefix, paying full price. GPT-5.6 and later models removed the 24-hour cache in favor of explicit cache controls. For highly async workflows, this gives more control than Anthropic's fixed 5-minute window.
...
- **Choose providers for async workflows.** OpenAI's configurable cache retention (GPT-5.x series) gives highly asynchronous workflows more control than Anthropic's fixed 5-minute window — though cache is best-effort, not guaranteed.
