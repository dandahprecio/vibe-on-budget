...
Before mid-2025, all schemas loaded every step. With Tool Search (Module 8), only needed tools load. Pruning unused MCP servers still matters — it shrinks the registry and reduces the chance of the model picking expensive tools for simple tasks.

See [Module 8 — Tool Search](/token-economy-course/en/08-under-the-hood) for how it works internally.
:::
...
Agent hygiene builds on [Project Setup](/token-economy-course/en/05-project-setup). For a broader view of managing AI-generated code quality, see [Vibe Coding Without the Debt](/token-economy-course/en/07-vibe-coding-guardrails).
:::
