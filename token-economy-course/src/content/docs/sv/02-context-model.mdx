...
Team Alpha upptäckte att deras "snabba fråga om autentisering" skickade 15 öppna flikar som kontext — ungefär 8 000 tokens från irrelevanta filer i varje förfrågan. Att förstå kontextmodellen minskade genast deras slöseri: 14 200 → 12 500 credits.
:::
...
## Förgrena konversationer

Skriv `/fork` i chattinmatningen för att skapa en ny session som **ärver hela konversationshistoriken**. Du kan också hovra över ett tidigare meddelande och välja **Fork Conversation** för att förgrena från just den kontrollpunkten.

Använd förgrening när du vill utforska en alternativ lösning eller ställa en sidofråga utan att betala prefixkostnaden igen. Till skillnad från `/clear` och `/compact` är förgrening till för divergerande utforskning — parallella spår — inte för att fortsätta samma uppgift i en mindre kontext.

Se [Fork a chat session](https://code.visualstudio.com/docs/agents/run/sessions/manage-sessions#_fork-a-chat-session) i VS Code-dokumentationen.

:::tip[Praktisk slutsats]
Förgrena innan du testar en riskfylld lösning: behåll originaltråden intakt och utforska alternativet i en gren som redan har den relevanta kontexten.
:::
...
## Så fungerar promptcachning

Promptcachning är den enskilt största drivkraften bakom tokenbesparingar i Copilot. När det gemensamma promptprefixet förblir stabilt kan leverantörer återanvända modellens redan beräknade tillstånd för systeminstruktioner, verktyg, repository-kontext och konversationshistorik i stället för att behandla dessa tokens från början varje gång.

### Cachemekanik hos olika leverantörer
...
|----------|-------------|--------------------|--------------|--------------|--------------|
| OpenAI (GPT-5.x) | Automatisk + manuell | Ja — `prompt_cache_breakpoint` | 1,25x | Ej offentligt specificerad | Konfigurerbar |
...
För de dagliga misstagen som bryter cachen, se avsnittet Vanliga misstag som bryter cachen ovan.

OpenAI GPT-5.x stöder konfigurerbar cacheretention (24 timmar som standard för organisationer utan ZDR på GPT-5-seriens modeller). Cachning sker efter bästa förmåga och är inte garanterad — ett anrop kan ibland hamna på en server utan ditt prefix och då betala fullt pris. GPT-5.6 och senare modeller tog bort 24-timmarscachen till förmån för uttrycklig cachekontroll. För mycket asynkrona arbetsflöden ger detta mer kontroll än Anthropics fasta femminutersfönster.
...
Om du använder Anthropic-baserade modeller (Claude i Copilot), håll sessionerna aktiva och sammanhängande. Långa pauser mellan stegen kan göra att du förlorar cachen. Om ditt arbetsflöde är mycket asynkront ger OpenAI:s konfigurerbara lagringsfönster i GPT-5.x dig mer kontroll.
:::

### Vad det innebär för dig

- **Ändra inte systeminstruktioner under sessionen.** Om du måste göra det, acceptera cachemissen som sker en gång och fortsätt.
- **Övervaka tysta cachefel.** Använd Copilots felsökningsloggning (`github.copilot.chat.agentDebugLog.fileLogging.enabled`) för att kontrollera faktiska tokenantal.
- **Välj leverantör utifrån asynkrona arbetsflöden.** OpenAI:s konfigurerbara cacheretention (GPT-5.x-serien) ger mycket asynkrona arbetsflöden mer kontroll än Anthropics fasta femminutersfönster — men cachning sker efter bästa förmåga och är inte garanterad.

För en genomgång av de dagliga misstagen som bryter cachen, se tabellen [Vanliga misstag som bryter cachen](#vanliga-misstag-som-bryter-cachen) ovan.

---

:::tip[Fortsätt lära dig]
Att förstå kontextmodellen gör siffrorna i [Den dolda skatten](/vibe-on-budget/sv/01-hidden-tax) konkreta. Tillämpa kunskapen med [Snabba vinster](/vibe-on-budget/sv/04-quick-wins).
:::
