...
flowchart TD
    subgraph Session[Agent-session]
        Step1["Steg 1: Planera<br/>System + Verktyg = 8K tokens"] --> Step2["Steg 2: Läs fil<br/>System + Verktyg = 8K tokens"]
        Step2 --> Step3["Steg 3: Skriv fil<br/>System + Verktyg = 8K tokens"]
        Step3 --> Step4["Steg 4: Kör tester<br/>System + Verktyg = 8K tokens"]
    end
    
    subgraph Overhead[Kostnad per steg]
        T[Verktygsdefinitioner<br/>skickas varje steg]
    end
...
Före mitten av 2025 laddades alla scheman vid varje steg. Med verktygssökning (modul 8) laddas bara nödvändiga verktyg. Det är fortfarande viktigt att rensa bort oanvända MCP-servrar — det gör registret mindre och minskar risken att modellen väljer dyra verktyg för enkla uppgifter.

Se [Modul 8 — Verktygssökning](/token-economy-course/sv/08-under-the-hood) för hur det fungerar internt.
:::
...
Se [Modul 2 — Cachebrytande misstag](/token-economy-course/sv/02-context-model#vanliga-cachebrytande-misstag) för fler mönster som bryter cachen.
...
:::tip[Fortsätt lära]
Agenthygien bygger vidare på [Projektinställningar](/token-economy-course/sv/05-project-setup). För ett bredare perspektiv på att hantera kvaliteten i AI-genererad kod, se [Vibe coding utan skuld](/token-economy-course/sv/07-vibe-coding-guardrails).
:::
