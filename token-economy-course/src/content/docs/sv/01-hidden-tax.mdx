...
Varje Copilot-förfrågan har tre token-kategorier. Men alla tokens kostar inte lika mycket. För att förstå vart dina credits faktiskt tar vägen börjar vi med volymen av varje kategori.

```mermaid
pie showData
    title Tokenvolymer per förfrågan
    "Output-tokens" : 35
    "Input-tokens" : 45
    "Cachade tokens" : 20
```
...
Men det finns en till dimension: **cache**. När ditt promptprefix — de delar av din förfrågan som upprepas mellan stegen — förblir stabilt kan Copilot återanvända tidigare beräkningar. Då betalar du upp till 90% mindre. Hur detta fungerar i praktiken går vi igenom i [Modul 2](/vibe-on-budget/sv/02-context-model).
...
:::tip[Auto-läge sparar automatiskt]
Håll modellväljaren på "Auto" när möjligt. Copilot skickar enkla frågor till billigare modeller och reserverar premium för komplexa uppgifter. Betalda planer får 10 % rabatt på modellkostnader vid automatiskt modellval.[^m1-7]
:::
...
1. Utbilda teamet om modellval (använd inte Opus för en strängdelning)
2. Övervaka credithförbrukning via GitHub admin-dashboard
...
:::note[Fördjupning: Effective Tokens-formeln]
GitHub använder **Effective Tokens (ET)** för att normalisera kostnader över tokentyper:[^m1-5]
...
Där:
- **m** — modellmultiplikator (använd 1,0 för en enkelmodellsbaslinje)
...
Output-tokens kostar ungefär **4–8× mer** än input-tokens per miljon. I kombination med 4,0× ET-vikten bränner ett utförligt modellsvar credits långt snabbare än en lång prompt.

GitHub rapporterade en **62 % minskning** av effektiva tokens över 109 körningscykler genom att strukturera om kontexten. Den här formeln och hur du maximerar cache-återanvändning går vi igenom i Modul 2.
...
Nu när du förstår kostnadsmodellen kan du läsa om [hur Copilot bygger kontext](/vibe-on-budget/sv/02-context-model) — den förklarar varför varje förfrågan kostar som den gör.
:::
