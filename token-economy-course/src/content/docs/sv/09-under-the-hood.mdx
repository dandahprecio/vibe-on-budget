...
VS Code-teamet fann att outputkomprimering ensam minskar fakturerbara output-tokens med **~5,5%** i genomsnitt. Tool Search (att skjuta upp verktygsscheman) sparar ytterligare **~9–11%** genom att läsa in fullständiga verktygsdefinitioner endast när de behövs. I kombination med code-only-instruktioner (Modul 4) kan output-tokens minska med 70%+.
...
Under 2026 flyttade GitHub Copilot i VS Code till **beständiga WebSocket-anslutningar** för agentiska sessioner. Detta minskar inte direkt tokenantalet, men det minskar **tiden till första token med ~19%** (median-TTFT) och eliminerar TLS-handskakningsomkostnaden vid varje steg.
